...
var ReconnectionHealthPolicy = /** @class */ (function (_super) {
    ...
    ReconnectionHealthPolicy.prototype.health = function () {
        ...
        var noPacketsReceivedRecently = this.currentData.consecutiveStatsWithNoPackets >=
            ReconnectionHealthPolicy.CONNECTION_UNHEALTHY_THRESHOLD;
        var noSignalStatusReceivedRecently = this.currentData.isNoSignalRecent(ReconnectionHealthPolicy.NO_SIGNAL_THRESHOLD_TIME_MS);
        ...
        var needsReconnect = noPacketsReceivedRecently ||
            noSignalStatusReceivedRecently ||
            missedPongsRecently ||
            hasBadAudioDelay;
        if (needsReconnect) {
            this.logger.warn("reconnection recommended due to: no packets received: " + noPacketsReceivedRecently + ", no signal status received: " + noSignalStatusReceivedRecently + ", missed pongs: " + missedPongsRecently + ", bad audio delay: " + hasBadAudioDelay);
            return 0;
        }
        return 1;
    };
