...
var DefaultScreenViewingDeltaRenderer = /** @class */ (function () {
    ...
    DefaultScreenViewingDeltaRenderer.prototype.renderSync = function () {
        ...
        for (var row = 0; row < this.imageDimensions.screenHeight; row++) {
            for (var col = 0; col < this.imageDimensions.screenWidth; col++) {
                var dx = col * this.imageDimensions.macroBlock;
                var dy = row * this.imageDimensions.macroBlock;
                var rendered = this.hasRendered[row][col];
                var jpegDataArray = this.jpegDataArrays[row][col];
                if (!jpegDataArray || rendered) {
                    continue;
                }
                var imageData = this.getImageData(jpegDataArray);
                if (!imageData) {
                    continue;
                }
